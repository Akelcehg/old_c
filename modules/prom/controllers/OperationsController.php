...
class OperationsController extends Controller
{
    ...
    public function actionAjaxcountertoconsumtiondetail()
    {
        ...
        $do=DateOptions::find()->where(['all_id'=>$counter_id])->orderBy(['id'=>SORT_DESC])->one();

        $dt=new \DateTime($endDate);
        $dt->add(new \DateInterval("P1DT".$do->contract_hour.'H'));

        $daydatas =DayData::find()->where(['all_id'=>$counter_id])
            ->andWhere(['>=','timestamp',$beginData.' '.$do->contract_hour.':00:00'])
        ...
        if($daydatas) {
            ...
            foreach ($daydatas as $daydata) {
                ...
                $indications += $daydata->v_sc;
            }
        }else{
            $hourdatas=HourData::find()->where(['all_id'=>$counter_id])
                ->andWhere(['>=','timestamp',$beginData.' '.$do->contract_hour.':00:00'])
                ->andWhere(['<','timestamp',$dt->format('Y-m-d H:i:s')])
                ->all();
            foreach ( $hourdatas as $hourdata) {


                $indications += $hourdata->v_sc;
            }
        ...
        $line.="<tr>";
        $line.="<td id='consumtionSumm'> <b>Расход : " . round($indications, 3) . "м<sup>3</sup></b></td>";
        $line.="</tr>";
        $line.="</table>";
        return $line;
    }
    ...
    public function actionAjaxcountertoconsumtiondetailbyweek()
    {
        ...
        for ($i = 1; $i <= 7; $i++) {
           ...
           // $last = MetrixCounter::getLastFlatIndicationsStatic($counter_id, $data[$i - 1], $data[$i]);
            ...
            $dt=new \DateTime($data[$i]);
            $dt->add(new \DateInterval("PT".$do->contract_hour.'H'));

            $daydatas =DayData::find()->where(['all_id'=>$counter_id])
                ->andWhere(['>','timestamp',$data[$i - 1].' '.$do->contract_hour.':00:00'])
                ->andWhere(['<=','timestamp',$dt->format('Y-m-d H:i:s')])
                ->one();
            if(isset($daydatas)) {
                $indication = $daydatas->v_sc;
            }else{
                $indication=0;
            }
            ...
            $line .=
                $title .
                '<td>'.$data[$i-1].'</td>'
                . '<td id="consumtionSumm"><b> Расход : ' . round($indication, 3) . 'м<sup>3</sup></b></td>';
