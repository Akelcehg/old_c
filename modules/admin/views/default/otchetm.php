<div style="width: 100%;height: 100%;" xmlns="http://www.w3.org/1999/html">
    ...
    <div style="width: 100%;text-align: center; margin-top:20px">
        ...
        <table style="width: 100%; border-bottom-color: #0a0a0a;border: 1; border-bottom-style: dashed;border-top-style: dashed;border-left: none;border-right: none ">
            ...
            foreach($daydata as $hour){
                ...
                ?>
            <tr><td><?=sprintf("%'02d",$hour->day).'.'.sprintf("%'02d",$hour->month).'.'.$hour->year?></td><td><?=number_format(round($hour->v_rc,2),2,'.','')?></td><td><?=number_format(round($hour->v_sc,2),2,'.','')?></td><td><?=number_format(round($hour->paverage,4),4,'.','')?></td><td><?=number_format(round($hour->taverage,2),2,'.','')?></td><td><?=$hour->emergency?></td></tr>
            <?php } ?>
            </tbody>
        </table>
    </div>
    <div style="width: 100%;text-align: left; margin-top:0px;margin-left: 90px">
        <span>
            <table >
                <tr>
                    <td width="57px">  <b>Итого:</b> </td> <td width="118px">   <b><?=number_format( $V_rc,2,'.','')?></b> </td><td>  <b><?=number_format($V_sc,2,'.','')?></b></td>
                </tr>
    ...
    <div style="width: 100%;text-align: left; margin-top:20px">
        <span>Безаварийный объем за месяц при р.у.,м3:<?=sprintf("%'.95s",number_format($V_rc,1,'.',''))?></span><br>
        <span>Аварийный объем за месяц при р.у.:<?=sprintf("%'.108s",number_format($avV,1,'.',''))?></span><br>
        <span>Полный объем за месяц при р.у.:<?=sprintf("%'.111s",number_format($V_rc,1,'.',''))?></span><br>
        <span>Безаварийный объем за месяц при c.у.,м3:<?=sprintf("%'.93s",number_format($V_sc,1,'.',''))?></span><br>
        <span>Аварийный объем за месяци при c.у.:<?=sprintf("%'.108s",number_format($avVsu,1,'.',''))?></span><br>
        <span>Полный объем за месяц при c.у.:<?=sprintf("%'.110s",number_format($V_sc+$avVsu,1,'.',''))?></span><br>
        ...
        <span>Общая длительность аварийніх ситуация за месяц , ч:мин:с:<?=sprintf("%'.55s%'02d:%'02d:%'02d","",$ta/3600,$ta%3600/60,$ta%3600%60)?></span><br>
    </div>
    <?php } else{ ?>
        <div style="width: 100%;text-align: left; margin-top:20px">
            ...
            <span>Полный объем за сутки при c.у.:<?=sprintf("%'.110s",number_format($V_sc+$avVsu,1,'.',''))?></span><br>
            <span>Длительность измирительных авар.ситуаций за месяц , ч:мин:с:<?=sprintf("%s%'02d:%'02d:%'02d","",$timeQem/3600,$timeQem%3600/60,$timeQem%3600%60)?></span><br>
            <span>Длительность методических авар.ситуаций за месяц , ч:мин:с:<?=sprintf("%s%'02d:%'02d:%'02d","",$timeQem/3600,$timeQem%3600/60,$timeQem%3600%60)?></span><br>
            <span>Длительность отключения питания за месяц , ч:мин:с:<?=sprintf("%'.10s%'02d:%'02d:%'02d","",$timeQem/3600,$timeQem%3600/60,$timeQem%3600%60)?></span><br>
            <span>Длит.постановки на несанкц. константы  за месяц , ч:мин:с:<?=sprintf("%'.10s%'02d:%'02d:%'02d","",$timeQem/3600,$timeQem%3600/60,$timeQem%3600%60)?></span><br>
            <span>Длит.работы когда расход был был < НПИ  за месяц , ч:мин:с:<?=sprintf("%'.10s%'02d:%'02d:%'02d","",$timeQem/3600,$timeQem%3600/60,$timeQem%3600%60)?></span><br>
            <span>Показания счетчика газа наконец отчетного периода (р.у.) , м3:<?=sprintf("%'.52s",number_format($pokaz,2,'.',''))?></span><br>
