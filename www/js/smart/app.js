//Fix shrinkWrapBlocks
if (jQuery.fn.jquery == '1.8.3') {
    jQuery(function() {
        jQuery.support.shrinkWrapBlocks = (function() {
            // Minified: var b,c,d
            var div, body, container, shrinkWrapBlocksVal;
...
$(document).ready(function() {
    ...
    // Init script for getting closest release notification time.
   // ReleaseNotification.init();
    ...
    $(document.body).tooltip({
        ...
        trigger: 'hover',
        selector: '[rel=popover-hover]'
    }).on("show.bs.popover", "[rel=popover-hover]", function() {
        forceRemovePopovers();
    });
    ...
    $('.minifyme').click(function(e) {
        ...
        if ($(document.body).hasClass('minified')) {
            $.cookie('sm_minified', '1', {
                path: '/'
            });
        } else {
            $.removeCookie('sm_minified', {
                path: '/'
            });
        }
    ...
    $('#hide-menu >:first-child > a').click(function(e) {
        $(document.body).toggleClass('hidden-menu');
        if ($(document.body).hasClass('hidden-menu')) {
            $.cookie('sm_hidemenu', '1', {
                path: '/'
            });
        } else {
            $.removeCookie('sm_hidemenu', {
                path: '/'
            });
        }
    ...
    // LOGOUT BUTTON
    $('#logout a,.logout_button').click(function(e) {
        //get the link
        var $this = $(this);
...
(function($, window, undefined) {
    ...
    $.event.special[str_resize] = {
        setup: function() {
            ...
            if (typeof elem[0].xAxis == 'undefined') {
                $.data(this, str_data, {
                    w: elem.width(),
                    h: elem.height()
                });
...
/**
 * Managing release notification functionality.
 */
var ReleaseNotification = {
    allowRequest: true,
    requestPeriod: 4 * 60 * 1000,
    minRequestPeriod: 2 * 60 * 1000,
    ...
    sendRequest: function() {
        $.ajax({
            type: "GET",
            url: '/site/getClosestReleaseNotification',
            complete: function(jqXHR, textStatus) {
                if (jqXHR.status == 200) {
                    var data = jQuery.parseJSON(jqXHR.responseText);
                    if (data && data.success) {
                        ReleaseNotification.requestPeriod = parseInt(data.request_period, 10);
                        ReleaseNotification.allowRequest = (data.allow_request == 1);
                        ReleaseNotification.currentDate = data.current_date;

                        if (!$.cookie('ReleaseNotification_sessionID') || $.cookie('ReleaseNotification_sessionID') != data.session_id) {
                            $.cookie('ReleaseNotification_sessionID', data.session_id, {
                                path: '/',
                                expires: 10
                            });
                            $.removeCookie('ReleaseNotification_firstRequestDone', {
                                path: '/'
                            });
                        }

                        if (data.allow_request == 1) {
                            if (data.release_after > -1) {
                                if (ReleaseNotification.dialogTimerId)
                                    clearTimeout(ReleaseNotification.dialogTimerId);

                                var logout = data.logout ? (data.logout == 1) : false;
                                var dialogType = data.dialog_type ? parseInt(data.dialog_type, 10) : 1;
                                if (dialogType == 2) {
                                    if (!$.cookie('ReleaseNotification_firstRequestDone') && (!$.cookie('ReleaseNotification_denyWarningAgain') || $.cookie('ReleaseNotification_denyWarningAgainDate') != ReleaseNotification.currentDate)) {
                                        data.dialog_text += "<br/><br/><input type='checkbox' onclick='ReleaseNotification.checkBoxClickHandle(this)'/>&nbsp;Do not show this warning again<br/>";
                                        $.cookie('ReleaseNotification_firstRequestDone', true, {
                                            path: '/',
                                            expires: 10
                                        });
                                    } else {
                                ...
                                ReleaseNotification.dialogTimerId = setTimeout('ReleaseNotification.showDialog("' + data.dialog_text + '", ' + logout + ')', parseInt(data.release_after, 10));
                            }
                        }
                    }
                }

                if (ReleaseNotification.allowRequest) {
                    if (ReleaseNotification.requestTimerId)
                        clearTimeout(ReleaseNotification.requestTimerId);

                    // be on the safe side
                    ReleaseNotification.requestPeriod = (ReleaseNotification.requestPeriod > ReleaseNotification.minRequestPeriod) ? ReleaseNotification.requestPeriod : ReleaseNotification.minRequestPeriod;

                    ReleaseNotification.requestTimerId = setTimeout('ReleaseNotification.sendRequest()', ReleaseNotification.requestPeriod);
                }
            }
        });
    },
    showDialog: function(dialogText, logout) {
        if (dialogText != '') {
            $('#alertBox .msgContent').html(dialogText);
    ...
    checkBoxClickHandle: function(obj) {
        if ($(obj).is(':checked')) {
            $.cookie('ReleaseNotification_denyWarningAgain', true, {
                path: '/',
                expires: 1
            });
            $.cookie('ReleaseNotification_denyWarningAgainDate', ReleaseNotification.currentDate, {
                path: '/',
                expires: 1
            });
        } else {
            $.removeCookie('ReleaseNotification_denyWarningAgain', {
                path: '/'
            });
            $.removeCookie('ReleaseNotification_denyWarningAgainDate', {
                path: '/'
            });
        }
    }
...
/**
 * Look about User working activity.
 */
var LookUserActivity = {
    allowRequest: true,
    isActive: 1,
    threshold: 20, // in minutes.    
    minThreshold: 5, // in minutes 
    checkPeriod: 1 * 1000,

    idleTime: 0,
    activeEvents: 'click keyup',

    init: function() {
        this.sendRequest(this.isActive);
        $(document).bind(LookUserActivity.activeEvents, LookUserActivity.processEvent);
        setTimeout(this.incrementTimer, this.checkPeriod);
    },

    processEvent: function() {
        LookUserActivity.idleTime = 0;
    },

    incrementTimer: function() {
        LookUserActivity.idleTime += 1.001;
        console.log(LookUserActivity.idleTime);
        if (LookUserActivity.idleTime >= LookUserActivity.threshold) {
            if (LookUserActivity.isActive != 0) {
                LookUserActivity.isActive = 0;
                LookUserActivity.sendRequest();
            }
        } else {
            if (LookUserActivity.idleTime == 1 && LookUserActivity.isActive != 1) {
                LookUserActivity.isActive = 1;
                LookUserActivity.sendRequest();
            }
        }

        if (LookUserActivity.allowRequest) {
            setTimeout(LookUserActivity.incrementTimer, LookUserActivity.checkPeriod);
        }
    },
    ...
    sendRequest: function() {
        ...
        $.ajax({
            type: "POST",
            url: '/site/setUserWorkingActivityStatus',
            data: {
                is_active: LookUserActivity.isActive
            },
            complete: function(jqXHR, textStatus) {
                if (jqXHR.status == 200) {
                    var data = jQuery.parseJSON(jqXHR.responseText);
                    if (data && data.success) {
                        LookUserActivity.threshold = parseInt(data.threshold, 10);
                        LookUserActivity.allowRequest = (data.allow_request == 1);
                        LookUserActivity.isActive = (data.is_active == 1);

                        // be on the safe side
                        if (LookUserActivity.threshold < LookUserActivity.minThreshold) {
                            LookUserActivity.threshold = LookUserActivity.minThreshold;
                        }

                        if (!LookUserActivity.allowRequest) {
                            $(document).unbind(LookUserActivity.activeEvents, LookUserActivity.processEvent);
                        }
                    }
                }
            }
    ...
    jarvismenu: function(options) {
            ...
            $this.find("li").each(function() {
                if ($(this).find("ul").size() != 0) {
                    ...
                    $(this).find("a:first").append("<b class='collapse-sig-n'>" + opts.closedSign + "</b>");
            ...
            $this.find("li a").click(function() {
                ...
                if ($(this).parent().find("ul").size() != 0) {
                    ...
                    if ($(this).parent().find("ul:first").is(":visible") && !$(this).parent().find("ul:first").hasClass("active")) {
                        //are we going to slideup -> check the link first
                        if ($(this).attr('href') != '' && $(this).attr('href') != '#' && this.href != window.location.href) {
                            return true; //prevent slideup animation
                        }
...
function clearDatepicker(item){
    if($.fn.datepicker){
        $(item || this).parent().find('.hasDatepicker[readonly]')
...
function limitStartDate(){
    if($.fn.datepicker){
        var self = $(this),
            ...
            form = $(this.form || document.body),
            elm = form.find(target);

        if(self.length && elm.length){
            elm.datepicker('option', 'minDate', self.datepicker('getDate'));
        }
    }
}
...
function limitEndDate(){
    if($.fn.datepicker){
        ...
        if(self.length && elm.length){
            elm.datepicker('option', 'maxDate', self.datepicker('getDate'));
        }
    }
}